...
import { tuckLogo as TuckLogo } from "@/components/ui/logo";
import { useAuth } from "@/contexts/AuthContext";

const navigationItems = [
  { title: "Dashboard", url: "/", icon: Home, roles: ['admin', 'cashier'] },
  { title: "Point of Sale", url: "/pos", icon: ShoppingCart, roles: ['admin', 'cashier'] },
  { title: "Inventory", url: "/inventory", icon: Package, roles: ['admin'] },
  { title: "Sales", url: "/sales", icon: Receipt, roles: ['admin'] },
      ...
      <SidebarContent>
        <div className="p-4 border-b border-sidebar-border">
          <div className="flex items-center gap-3">
            <TuckLogo size="md" className="text-tucktical-gold flex-shrink-0" />
            {!isCollapsed && (
        ...
        {user && !isCollapsed && (
          <div className="mt-auto p-4 border-t border-sidebar-border">
            <div className="text-sm text-sidebar-foreground/70">
              ...
              <div className="capitalize text-tucktical-accent truncate">{user.role}</div>
            </div>
          </div>
        )}
      </SidebarContent>
